        ...
        observation_info={
            **{
                k.split(".")[-1]: tfds.features.Image(
                    ...
                    shape=[*v["shape"][1:], v["shape"][0]], dtype=np.uint8, encoding_format=encoding_format, doc=v["names"]
                )
                for k, v in features.items()
...
def parse_step(data_item):
    observation_info = {
        ...
        **{"_".join(k.split(".")[2:]) or k.split(".")[-1]: v for k, v in data_item.items() if "observation.state" == k},
    }
    action_info = {
    ...
    # print("====== action:", action_info.keys())
    if action_info["action"][7] > 1500:
        action_info["action"][7] = 1500
    if action_info["action"][-1] > 1500:
        action_info["action"][-1] = 1500
    if observation_info["state"][7] > 1500:
        observation_info["state"][7] = 1500
    if observation_info["state"][-1] > 1500:
        observation_info["state"][-1] = 1500

    # print("====== action:", action_info.keys())
    # print("====== state:", observation_info.keys())
